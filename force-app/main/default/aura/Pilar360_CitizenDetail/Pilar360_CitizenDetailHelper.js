({
        ...
        if(strRecordId !== undefined && strRecordId !== null && (strRecordId.length === 15 || strRecordId.length === 18)) {
            ...
            objControllerAction.setCallback(this, function(objResponse) {
                ...
                if (objComponent.isValid() && strState === "SUCCESS") {
                    ...
                    console.log('celular: ' + objCitizenWrapper.pilar360.mobile_phone);
                    console.log('email: ' + objCitizenWrapper.pilar360.email);
                    console.log('fijo: ' + objCitizenWrapper.pilar360.phone);

                    objComponent.set('v.citizenWrapper', objCitizenWrapper );

                    objComponent.set('v.mobile', objCitizenWrapper.pilar360.mobile_phone );
                    objComponent.set('v.email', objCitizenWrapper.pilar360.email );
                    objComponent.set('v.fijo', objCitizenWrapper.pilar360.phone );
                } 
            });
    ...
    makeLog : function(objComponent) {
        var action = objComponent.get("c.createLog");
        action.setParams({ recordId : objComponent.get("v.recordId") });
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state !== "SUCCESS") {
                console.log('ocurrio un error tratando de generar el log')
            }
        });
