({
    initializeComponent: function(objComponent, objEvent, objHelper) {
        objComponent.set('v.iscontacttracing', (window.location.pathname.indexOf('misiones-contact-tracing') > 0 ) );

    },
    contactLink : function(objComponent, objEvent, objHelper) {
        var urlString = window.location.href;
        var baseURL = urlString.substring(0, urlString.indexOf("/s"));
        var objCitizenWrapper = objComponent.get('v.citizenWrapper');
        var finalURL = baseURL + '/s/contact/' + objCitizenWrapper.citizen.Id;
        // console.log('contact: ' + finalURL);
        window.location.replace(finalURL);
    },
    tracingLink : function(objComponent, objEvent, objHelper) {
        var urlString = window.location.href;
        var baseURL = urlString.substring(0, urlString.indexOf("/s"));
        var objCitizenWrapper = objComponent.get('v.citizenWrapper');
        var finalURL = baseURL + '/s/misiones-contact-tracing/?id=' + objCitizenWrapper.citizen.Id;
        // console.log('tracing: ' + finalURL);
        window.location.replace(finalURL);
    },
