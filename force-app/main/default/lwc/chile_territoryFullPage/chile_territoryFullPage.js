...
import retrieveTerritory from '@salesforce/apex/TerritoryDetailChileController.retrieveTerritory';
import getTerritoryById from '@salesforce/apex/TerritoryDetailChileController.getTerritoryById';
...
export default class Newcom_territoryFullPage extends LightningElement {
    ...
    wiredTerritory({ error, data }) {
        if (data) {
            ...
            // this.iconPeople = Resources + '/img/IconPeople.png';
    ...
    wiredTerritory2({ error, data }) {
        if (data) {
            ...
            // set nivel adminitrativo
            if(this.territoryInfo.administrativelevel=="0"){
                this.nivelAdministrativo0 = true;
            } else if(this.territoryInfo.administrativelevel=="1"){
                this.nivelAdministrativo1 = true;
            } else if(this.territoryInfo.administrativelevel=="2"){
                this.nivelAdministrativo2 = true;
            } else if(this.territoryInfo.administrativelevel=="3"){
                this.nivelAdministrativo3 = true;
            }

            console.error('Error en el getTerritoryById');
            console.error(error);
        ...
        fetch(strUrl).then((objResponse) => objResponse.json()).then((objData) => {
            if(objData !== undefined && objData !== null && objData.length > 0) {
                ...
                var greenIcon = L.icon({
                    ...
                    // shadowUrl: RESOURCES + '/img/mail.svg',
                ...
                // this.template.querySelector(".map").style.display = "block";

            } else {
                // this.template.querySelector(".map").style.display = "None";
                // this.showMap = false;
                // intZoom -= 2;
