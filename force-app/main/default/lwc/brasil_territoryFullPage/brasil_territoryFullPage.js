...
export default class brasil_territoryFullPage extends LightningElement {
    recordcount = 5;
    ...
    wiredTerritory({ error, data }) {
        if (data) {
            ...
            // education levels
            this.educationLevels.push({"description": "Ensino fundamental","image": this.iconEduPrimaria,"name": this.territoryInfo.educationLevels[0].info.name,"value":this.territoryInfo.educationLevels[0].info.value});
            this.educationLevels.push({"description": "Ensino médio","image": this.iconEduSecundaria,"name": this.territoryInfo.educationLevels[1].info.name,"value":this.territoryInfo.educationLevels[1].info.value});
            this.educationLevels.push({"description": "Ensino técnico","image": this.iconEduTerciaria,"name": this.territoryInfo.educationLevels[2].info.name,"value":this.territoryInfo.educationLevels[2].info.value});
            this.educationLevels.push({"description": "Universidade completa","image": this.iconEduUniversitaria,"name": this.territoryInfo.educationLevels[3].info.name,"value":this.territoryInfo.educationLevels[3].info.value});
            ...
            // income ranges
            this.incomeranges.push({"name": "BAIXO","description": "menos de dois SM", "image": this.iconIngresoBajo, "value": (parseInt(strIngresoMuyBajo) + parseInt(strIngresoBajo)).toLocaleString()});
            this.incomeranges.push({"name": "MÉDIO","description": "entre dois e seis SM", "image": this.iconIngresoMedio, "value": (parseInt(strIngresoMedio)).toLocaleString()});
            this.incomeranges.push({"name": "ALTO","description": "mais de seis SM", "image": this.iconIngresoAlto, "value": (parseInt(strIngresoAlto) + parseInt(strIngresoMuyAlto)).toLocaleString()});
            // console.log('income ranges:', this.incomeranges);

            // occupation types
            this.occupationTypes.push({"name": "PESSOAL ASSALARIADO", "image": this.iconOcuAsalariado, "description": this.territoryInfo.occupationTypes[0].info.description, "value": this.territoryInfo.occupationTypes[0].info.value});
            this.occupationTypes.push({"name": "TRABALHADORES POR CONTA PRÓPRIA", "image": this.iconOcuCuentapropista, "description": this.territoryInfo.occupationTypes[1].info.description, "value": this.territoryInfo.occupationTypes[1].info.value});
            this.occupationTypes.push({"name": "APOSENTADOS", "image": this.iconOcuJubilado, "description": this.territoryInfo.occupationTypes[2].info.description, "value": this.territoryInfo.occupationTypes[2].info.value});
            this.occupationTypes.push({"name": "SEM RENDA DECLARADA", "image": this.iconOcuSinIngreso, "description": this.territoryInfo.occupationTypes[3].info.description, "value": this.territoryInfo.occupationTypes[3].info.value});
                        ...
                        ticks: {
                            callback: function(label, index, labels) {
                                if(label>1000000) {
                                    ...
                                    var miles = label/1000 < 1000 ? ' milhares' : ' milhão';
                                    var cantidad = label/1000 < 1000 ? label/1000 : label/1000000;
                                    return cantidad+miles;
                                } else {
                                    if(label<=1){
                                        return label*10 + ' milhares';
                                    } else {
                                        return label/1000 + ' milhares';
                                    }
                                }
                            }
                        }    
