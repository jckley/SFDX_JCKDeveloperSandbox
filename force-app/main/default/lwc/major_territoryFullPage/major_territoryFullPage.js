...
export default class Newcom_territoryFullPage extends LightningElement {
    ...
    wiredTerritory({ error, data }) {
        if (data) {
            ...
            // ICONS ASSIGNMENT
            this.iconTerritories = Resources + '/img/Territories/territorio.svg';
            this.iconMarker = Resources + '/img/Territories/marker.svg';
            this.iconMapMarker = Resources + '/img/Territories/territorio.svg';
            this.iconIngresoBajo = Resources + '/img/Territories/indicador_bajo.svg';
            this.iconIngresoMedio = Resources + '/img/Territories/indicador_medio.svg';
            this.iconIngresoAlto = Resources + '/img/Territories/indicador_alto.svg';
            this.iconOcuAsalariado = Resources + '/img/Territories/icon_asalariado.svg';
            this.iconOcuCuentapropista = Resources + '/img/Territories/icon_cuentaprop.svg';
            this.iconOcuJubilado = Resources + '/img/Territories/icon_jubilado.svg';
            this.iconOcuSinIngreso = Resources + '/img/Territories/icon_siningresos.svg';
            this.iconEduPrimaria = Resources + '/img/Territories/ne_primario.svg';
            this.iconEduSecundaria = Resources + '/img/Territories/ne_secundario.svg';
            this.iconEduTerciaria = Resources + '/img/Territories/ne_universitario.svg';
            this.iconEduUniversitaria = Resources + '/img/Territories/ne_universitario_completo.svg';
            this.iconEmpleado = Resources + '/img/Territories/ae_empleado.svg';
                ...
                if(this.territoryInfo.geographicdistributions.length > 0) {
                    ...
                    for (var gd of this.territoryInfo.geographicdistributions) {
                        if(gd.info.link.includes('/prosumia/')){
                            var id= gd.info.link.substring(10);
                            console.log('ID de GD', id);
                            console.log(this.ORIGIN_PAGE)
                            var link = this.ORIGIN_PAGE+'/s/territorio-administrativo/'+id;
    ...
    wiredTerritory2({ error, data }) {
        if (data) {
            ...
            var admLevelLink = this.ORIGIN_PAGE+'/s/territorio-administrativo/';
            this.array_adminLevel = [];
            if(this.territoryWrapper.ter1.Name && !this.nivelAdministrativo1){
                this.array_adminLevel.push({ "id": this.territoryWrapper.ter1.Id, "name":this.territoryWrapper.ter1.Name, "link":admLevelLink+this.territoryWrapper.ter1.Id, "isLast":false })
            }
            if(this.territoryWrapper.ter2.Name && !this.nivelAdministrativo2){
                this.array_adminLevel.push({ "id": this.territoryWrapper.ter2.Id, "name":this.territoryWrapper.ter2.Name, "link":admLevelLink+this.territoryWrapper.ter2.Id, "isLast":false })
            }
            if(this.territoryWrapper.ter3.Name && !this.nivelAdministrativo3){
                this.array_adminLevel.push({ "id": this.territoryWrapper.ter3.Id, "name":this.territoryWrapper.ter3.Name, "link":admLevelLink+this.territoryWrapper.ter3.Id, "isLast":false })
            }
            if(this.territoryWrapper.ter4.Name && !this.nivelAdministrativo4){
                this.array_adminLevel.push({ "id": this.territoryWrapper.ter4.Id, "name":this.territoryWrapper.ter4.Name, "link":admLevelLink+this.territoryWrapper.ter4.Id, "isLast":false })
            }
                    ...
                    yAxes: [{
                        ticks: {
                            callback: function(label, index, labels) {
                                if(label>1000000){
                                    return label/1000000+'MM';
                                }else if(label>1000){
                                    return label/1000+'mm';
                                }else{
                                    return label;
                                }
                            }
    ...
    drawingMap(strAddress){
        ...
        fetch(strUrl).then((objResponse) => objResponse.json()).then((objData) => {
            if(objData !== undefined && objData !== null && objData.length > 0) {
                ...
                objMap = L.map(mapRoot).setView([dblLatitude, dblLongitude], intZoom)
